...
public class RemoteCommandUtil {
    ...
    public static Boolean loginByFileKey(RemoteConnect remoteConnect, File keyFile, String keyfilePass) {
        boolean flag = false;
        // 输入密钥所在路径
        //File keyfile = new File("C:\\ycx100527");
        try {
            conn = new Connection(remoteConnect.getIp());
            conn.connect();
            // 登录认证
     ...
     * @return
     */
    public static Boolean loginByFileKey(RemoteConnect remoteConnect, String keyfilePass) {
        boolean flag = false;
        // 输入密钥所在路径
     ...
     * @throws IOException
     */
    public static void scpPut(RemoteConnect remoteConnect,String localFile, String remoteDir) throws IOException{
        File keyfile = new File(remoteConnect.getFilePath());
        if(loginByFileKey(remoteConnect,keyfile,null)){
            SCPClient client = new SCPClient(conn);
            client.put(localFile, remoteDir);
            conn.close();
        }
